...
class CircuitOverview(Slide):
    def construct(self):
        ...
        curra = Arrow(start=resistor.get_terminals("left"), end=resistor.get_terminals("right"), tip_length=0.2).scale(.5).shift(UP*.5)
        ...
        currb =  Arrow(start=resistor.get_terminals("right"), end=resistor.get_terminals("left"), tip_length=0.2).scale(.5).shift(DOWN*.5)
        ...
        short = Tex(r"Short Circuit $R \rightarrow 0$").next_to(vgrp, DOWN, buff=.2).scale(.8)
        self.play(FadeIn(vgrp), Write(short))
        linx = Line(dota.get_center()+[0,-.5,0], dota.get_center()+[0.25,-.5,0])
        liny = Line(dotb.get_center()+[0,-.5,0], dotb.get_center()+[-0.25,-.5,0])
        dotx = Dot(dota.get_center()+[0.25,-.5,0])
        doty = Dot(dotb.get_center()+[-0.25,-.5,0])
        vgrp2 = VGroup(linx, liny, dotx, doty).next_to(vgrp, RIGHT, buff=4.25)
        openc = Tex(r"Open Circuit $R \rightarrow \infty$").scale(.8).next_to(vgrp2, DOWN, buff=.25)
        self.play(FadeIn(vgrp2), Write(openc))
        self.next_slide()
        
        self.play(*[FadeOut(mob) for mob in self.mobjects])
        title2= Tex(r"Passive Sign Convention").to_edge(UP)
        self.play(Write(title2))
        elemx = Tex(r"X").scale(.75)
        dot = Dot()
        dot2 = Dot().move_to(dot.get_center()+[0,-2,0])
        circle = Circle(radius=.5, color=WHITE).shift(RIGHT*1.5+DOWN*1)
        elemx.move_to(circle.get_center())
        line1 = Line(dot.get_center(), dot.get_center() + [1.5,0,0])
        line2 = Line(dot.get_center() + [1.5,0,0], dot.get_center() + [1.5,-.5,0])
        line3 = Line(dot2.get_center(), dot2.get_center() + [1.5,0,0])
        line4 = Line(dot2.get_center() + [1.5,0,0], dot2.get_center() + [1.5,.5,0])
        curra = Arrow(start=line1.get_center()+[0,.25,0], end=line1.get_center()+[1,.25,0], tip_length=0.1).scale(.5)
        labx = Tex(r"$I_{AB}$").scale(.6).next_to(curra, UP, buff=.2)
        laba = Tex(r"A\\ +").move_to(dot.get_center())
        labb = Tex(r"-\\ B").move_to(dot2.get_center()).shift(DOWN*.1)
        vgrp4 = VGroup().add(line1,line2,line3,line4,dot,dot2,circle,elemx, curra, laba, labb, labx).shift(UP+LEFT*.5)
        self.play(FadeIn(vgrp4))
        text = Tex(r"$I_{AB}$ is positive $\rightarrow$ positive charge.\\X absorbes energy and power").next_to(title2, DOWN, buff=.2)
        self.wait()
